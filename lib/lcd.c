...
void lcd_init(void)//Initializes LCD
{
   ...
   //---------one------
	...
	_delay_ms(1);
	//--------8 bit dual line----------
	lcd_send_command(0b00111000); //8 bit mode
   ...
   //-----increment address, invisible cursor shift------
	...
	while(ch<64)
	{
		lcd_define_char((LcdCustomChar+ch),chn++);
		ch=ch+8;
	}
}	
void lcd_send_char(uint8_t ch)		//Sends Char to LCD
{
...
void lcd_send_command(uint8_t cmd)	//Sends Command to LCD
{
#ifdef LCD_4BIT	
	//4 bit part
	LDP=(cmd & 0b11110000);
...
//lcd_copy_str(welcomeln1, 3, 1);	
void lcd_copy_str(const uint8_t *FlashLoc, uint8_t x, uint8_t y)
{
	uint8_t i;
	lcd_goto_xy(x,y);
	for(i=0;(uint8_t)pgm_read_byte(&FlashLoc[i]);i++)
	{
		lcd_send_char((uint8_t)pgm_read_byte(&FlashLoc[i]));
	}
}
...
lcd_define_char(backslash,0);
*/
void lcd_define_char(const uint8_t *pc,uint8_t char_code){
	uint8_t a, pcc;
	uint16_t i;
	a=(char_code<<3)|0x40;
	for (i=0; i<8; i++){
